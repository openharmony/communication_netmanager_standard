...
bool StaticConfiguration::Marshalling(Parcel &parcel) const
{
    if (!ipAddr_.Marshalling(parcel)) {
        NETMGR_LOG_E("write ipAddr_ to parcel failed");
        return false;
    }
    if (!route_.Marshalling(parcel)) {
        NETMGR_LOG_E("write route_ to parcel failed");
        return false;
    }
    if (!gate_.Marshalling(parcel)) {
        NETMGR_LOG_E("write gate_ to parcel failed");
        return false;
    }
    if (!netMask_.Marshalling(parcel)) {
        NETMGR_LOG_E("write netMask_ to parcel failed");
        return false;
    }
    if (!parcel.WriteUint32(dnsServers_.size())) {
        NETMGR_LOG_E("write dnsServers_ size to parcel failed");
        return false;
    }
    for (auto it = dnsServers_.begin(); it != dnsServers_.end(); ++it) {
        if (!it->Marshalling(parcel)) {
            NETMGR_LOG_E("write dnsServers_ to parcel failed");
            return false;
        }
    }
    if (!parcel.WriteString(domain_)) {
        NETMGR_LOG_E("write domain_ to parcel failed");
        return false;
    }
    return true;
}
